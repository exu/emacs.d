...
(defun org-open-go-workshops ()
  (interactive)
  (find-file (expand-file-name "~/org/go-workshops.org")))

(defun org-open-wiki ()
  (interactive)
  (find-file (expand-file-name "~/org/wiki/")))

(defun org-open-workshops ()
  (interactive)
  (find-file (expand-file-name "~/org/go-workshops.org")))
...
(defun open-recent-sql-file ()
  (interactive)
  (find-file (expand-file-name "~/src/run.sql")))
...
(cdr (cdr (split-string "/home/exu/src/jajaj/jajaja/src/jkokokkook/kokokkok" "/src/")))
...
(defun php-symfony2-generate-namespace ()
  ...
  (setq namespace
        (replace-regexp-in-string "\/$" ""
          (car (cdr (cdr (split-string (file-name-directory (or load-file-name buffer-file-name)) "/src/"))))))
...
(defun highlight-symbol-on-click (πclick)
  "Mouse click to `highlight-symbol-at-point' at clicked point."
  (interactive "e")
...
(defun kgo-tpl-clean ()
  ...
  ;; arrays ''=>'' --> '' => ''
  ;; (replace-regexp "\\([^\s]\\)\\(=>\\)" "\\1 =>" nil (point-min) (point-max))
  (replace-regexp "przepis.id" ".Id" nil (point-min) (point-max))
  (replace-regexp "przepis.nazwa" ".Name" nil (point-min) (point-max))
  (replace-regexp "przepis.opis" ".Description" nil (point-min) (point-max))
  (replace-regexp "przepis.sklad" ".Ingredients" nil (point-min) (point-max))
  (replace-regexp "przepis.przygotowanie" ".Preparation" nil (point-min) (point-max))
  (replace-regexp "przepis.mediumMedium" ".MainImage.Path.Medium" nil (point-min) (point-max))
  (replace-regexp "przepis.big" ".MainImage.Path.Big" nil (point-min) (point-max))
  (replace-regexp "przepis.imageDescription" ".MainImage.Description" nil (point-min) (point-max))
