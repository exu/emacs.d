...
;; org-impress-js.el tested on Emacs 24.0.92.1 and
;; Org-mode 7.8.11.
...
(defcustom org-export-impress-js-date-format-string "%Y-%m-%dT%R%z"
...
(defun org-impress-js-handle-links (org-line opt-plist)
...
PUB-DIR is set, use this as the publishing directory."
  ...
  (let* ((opt-plist
    ...
    (cond
     ...
     (date)
     (t (setq date (format-time-string org-export-impress-js-date-format-string))))
...
<div id=\"title\" class=\"step slide\" data-x=\"0\" data-y=\"0\">"
	...
	(catch 'nextline
	  ...
	  (when inquote
	    (insert (org-impress-js-protect org-line) "\n")
	    (throw 'nextline nil))
	  ...
	  (when (and org-export-with-fixed-width
	    ...
	    (insert (org-impress-js-protect (match-string 3 org-line)) "\n")
	    (when (or (not lines)
		      (not (string-match "^[ \t]*:\\(\\([ \t]\\|$\\)\\(.*\\)\\)"
					 (car lines))))
	      (setq infixed nil)
	  ...
	  (setq org-line (org-impress-js-handle-time-stamps org-line))
	  ...
	  (or (string-match org-table-hline-regexp org-line)
	      (string-match "^[ \t]*\\([+]-\\||[ ]\\)[-+ |]*[+|][ \t]*$" org-line)
	      (setq org-line (org-impress-js-expand org-line)))

	  ;; Format the links
	  (setq org-line (org-impress-js-handle-links org-line opt-plist))
	  ...
	  (cond
	   ...
	   (t
	    ...
	    (when (get-text-property 0 'list-item org-line)
	      (setq org-line (org-impress-js-export-list-line
			  org-line
...
When TITLE is nil, just close all open levels."
  ...
  (let* ((target (and title (org-get-text-property-any 0 'target title)))
	 ...
	 (data-rotate-x (and title (org-get-text-property-any 0 'data-rotate-x title)))
	 (data-rotate-y (and title (org-get-text-property-any 0 'data-rotate-y title)))
	...
	(insert (format "\n<div id=\"outline-container-%s\" class=\"outline-%d%s %s\" data-x=\"%s\" data-y=\"%s\" data-z=\"%s\" data-rotate=\"%s\" data-rotate-x=\"%s\" data-rotate-y=\"%s\" data-scale=\"%s\">\n<header><h%d id=\"%s\">%s%s</h%d></header>\n<section>\n<div class=\"outline-text-%d\" id=\"text-%s\">\n"
			suffix level (if extra-class (concat " " extra-class) "")
			step data-x data-y data-z data-rotate data-rotate-x data-rotate-y data-scale (or 1 level) href
			extra-targets
...
(defun org-impress-js-export-list-line (org-line pos struct prevs)
...
(defun org-impress-js-vec3-rotate-x (vec r)
  ...
  (destructuring-bind (sx sy sz) vec
    (let ((dx sx)
	  (dy (+ (* sy (cos r)) (* sz (* -1 (sin r)))))
	  (dz (+ (* sy (sin r)) (* sz (cos r)))))
      `(,dx ,dy ,dz))))

(defun org-impress-js-vec3-rotate-y (vec r)
  ""
  (destructuring-bind (sx sy sz) vec
    (let ((dx (+ (* sx (cos r)) (* sz (sin r))))
	  (dy sy)
	  (dz (+ (* sx (* -1 (sin r))) (* sz (cos r)))))
      `(,dx ,dy ,dz))))

(defun org-impress-js-vec3-rotate-z (vec r)
  ""
  (destructuring-bind (sx sy sz) vec
    (let ((dx (+ (* sx (cos r)) (* sy (* -1 (sin r)))))
	  (dy (+ (* sx (sin r)) (* sy (cos r))))
	  (dz sz))
      `(,dx ,dy ,dz))))
...
(defun org-impress-js-vec3-scale (vec scvec)
  ""
  (destructuring-bind (sx sy sz) vec
    (destructuring-bind (scx scy scz) scvec
      (let ((dx (* sx scx))
	    (dy (* sy scy))
	    (dz (* sz scz)))
	`(,dx ,dy ,dz)))))
...
(defconst org-export-impress-js-default-page-width 1024
  "")

(defconst org-export-impress-js-default-page-height 768
  "")

(defconst org-export-impress-js-default-page-depth 300
  "")

(defconst org-export-impress-js-default-rotation-degree 30
  "")

(defun org-export-impress-js-org-entry-get (entry default)
  ""
  (let ((v (org-entry-get (point) entry)))
    (if v
	(if (string-equal (format "%s" v) "t")
	    default
	  (org-export-impress-js-number v))
      0)))

(defun org-impress-js-save-entry-properties ()
  ""
  (let ((rvec `(,0 ,0 ,0))
	(data-x 0) (data-y 0) (data-z 0)
	(data-rotate-x 0) (data-rotate-y 0) (data-rotate 0)
	(data-scale 1))
    (org-map-entries
     (lambda ()
       (let (move-x move-y move-z)

	 (setq move-x
	       (org-export-impress-js-org-entry-get
		"move-x"
		org-export-impress-js-default-page-width))
	 (setq move-y
	       (org-export-impress-js-org-entry-get
		"move-y"
		org-export-impress-js-default-page-height))
	 (setq move-z
	       (org-export-impress-js-org-entry-get
		"move-z"
		org-export-impress-js-default-page-depth))

	 (and (zerop move-x) (zerop move-y) (zerop move-z)
	      (setq move-x org-export-impress-js-default-page-width))

	 (let ((v (org-entry-get (point) "no-move")))
	   (and v (setq move-x 0) (setq move-y 0) (setq move-z 0)))
	 ...
	 (destructuring-bind (x y z)
	     (let ((v (let ((rx (org-export-impress-js-org-entry-get
				 "rotate-x"
				 org-export-impress-js-default-rotation-degree))
			    (ry (org-export-impress-js-org-entry-get
				 "rotate-y"
				 org-export-impress-js-default-rotation-degree))
			    (rz (org-export-impress-js-org-entry-get
				 "rotate"
				 org-export-impress-js-default-rotation-degree)))
